...
const Navbar = ({ onReverseOrder, onListItemSelect, onDateSelect }) => {
  ...
  const handleDateChange = (date) => {
    ...
    onDateSelect(date); // Pass the selected date to the App component
  ...
  console.log("selectedDate in Navbar",selectedDate)

  return (
    <div
      className={`navbar navbarr mt-4 ${
        isDarkMode ? "dark" : "bg-gray-200"
      } w-full lg:w-4/5 px-4 py-2 mx-auto`}
    >
      <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 justify-between items-center">
        ...
        <div className="relative flex items-center w-full sm:w-auto">
          <div className="searchBar">
            <input
              ...
              value={searchText}
              className="ip-field1 p-2 pr-12 sm:pr-16 rounded border border-gray-400 focus:outline-none focus:border-blue-500 w-full"
          ...
          {listPlace.length > 0 && (
            <ul
              className="list1 absolute left-0 mt-8 w-full bg-white border-gray-300 rounded"
              style={{
                zIndex: 9999,
                top: inputHeight + 10,
            ...
            >
              {uniqueListPlace.map((place) => (
                <li
                  ...
                  className="list-item1 p-2 cursor-pointer hover:bg-gray-200"
                >
        ...
        <div className="relative flex items-center w-full sm:w-auto">
          <input
            ...
            className="ip-field2 p-2 pr-8 sm:pr-10 rounded border border-gray-400 focus:outline-none focus:border-blue-500 w-full"
            value={formattedDate}
            readOnly
          />
        ...
        <div className="lg:mr-20 flex lg:space-x-6 sm:space-x-2 space-x-6">
          ...
          <div className="flex items-center">
            {/* Conditionally render IoMoon or IoSunny based on dark mode */}
            <button
              className="px-3 py-2 border border-black rounded-full"
              onClick={toggleDarkMode}
            >
              {isDarkMode ? (
                <IoSunny className="text-lg sm:text-xl text-blue-500" />
              ) : (
